...
import os, re, sys, math, time, socket, ConfigParser, json
import logging
...
def parse_and_queue_datagram(line, host, queue):
    parsed = {}
    ...
    with producers[connection].acquire(block=True) as producer:
        maybe_declare(Exchange(queue, type="direct"), producer.channel)
        producer.publish(payload, routing_key="")
...
if __name__ == '__main__':
    ...
    for a in xrange(config.getint('amqp', 'connection_pool_size')):
    ...
    while True:
        ...
        gevent.spawn(parse_and_queue_datagram, data, peer, config.get('amqp', 'queue'))
